...
export function parseSpawnSummerArgs(argv) {
    let ticketId;
    let ticketPath;
    let ticketFile;
...
async function main() {
    ...
    printMinimalPanel('Spawning Summer QA Agent', {
        Request: parsed.task,
    ...
    const spinnerTimer = setInterval(() => {
        ...
        process.stdout.write(`\r   ${Style.MAGENTA}${spinChar}${Style.RESET} Summer Auditing... ${Style.DIM}[${formatTime(elapsed)}]${Style.RESET}\x1b[K`);
        spinnerIdx++;
    }, 100);
    proc.on('close', (code) => {
