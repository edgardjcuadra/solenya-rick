...
2.  **Conductor Check**: Use `glob` or `list_directory` to check the `conductor/` directory for existing `product.md` or track definitions. If they exist, pull the PRD or context per your god-tier judgment.
3.  **Temporary Spec Practice**: If the request is complex or Conductor lacks details, create a "Temporary Spec" in `${SESSION_ROOT}/temp_spec.md` to reason through the problem, draft plans, and ensure all tickets will be fully covered.
4.  **Fast Track**: If the prompt is specific (e.g., "Add a 'Copy' button to the code block component"), **SKIP INTERROGATION** and draft the PRD immediately based on the Temporary Spec or prompt.
5.  **Interrogate Yourself**: If the request is vague (e.g., "Fix the UI"), do NOT ask the user questions. Instead, infer the most reasonable answers and choose the best option.
    -   **The "Why"**: Infer the user problem and business value.
    -   **The "What"**: Infer specific scope and constraints.
6.  **Identify Points of Interest**: If needed, infer likely file pointers or components based on repo structure or prior context.
