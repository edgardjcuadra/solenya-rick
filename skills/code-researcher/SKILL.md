...
- **Execute Research (Specialized Roles)**:
  - **The Locator**: Use `deep_research` (from MCP `open-aware`) or `glob` to find WHERE files and components live.
  - **The Analyzer**: Use `ask` (from MCP `open-aware`) or read identified files to understand HOW they work. Trace execution.
  - **The Pattern Finder**: Use `get_context` (from MCP `open-aware`) or `search_file_content` to find existing patterns to model after.
