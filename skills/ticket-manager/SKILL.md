...
2.  **Consult Temporary Specs**: Read `${SESSION_ROOT}/temp_spec.md` if it exists. Use this scratchpad to reason about the tickets, ensuring no architectural blind spots exist before writing the final ticket files.

3.  **Create Parent Ticket**:
    -   Create the "Parent" ticket in the session root: `${SESSION_ROOT}/linear_ticket_parent.md`.
    -   Status: "Backlog" or "Research Needed".
    -   Title: "[Epic] [Feature Name]".
    -   Links: Add link to PRD and `temp_spec.md` if applicable.

4.  **Create Child Tickets (ATOMIC IMPLEMENTATION)**:
    -   Break the PRD/Spec into atomic implementation tasks (e.g., "Implement Backend API", "Develop Frontend UI", "Integrate Services").
    ...
    -   **Execution Order**: Respect the "Implementation Plan" and "Phases & Ticket Order" defined in the PRD or Temporary Spec.
    ...
    -   **Leave Audit State**: If instructed or if Solenya mode is active, log an audit of the tickets created into a `conductor/audit_state.log` or `${SESSION_ROOT}/audit.log`.
